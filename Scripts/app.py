...
# EXTRACT DATA ROW SAFELY USING .iloc[0] TO PREVENT TYPEERRORS
village_data = df[df['NAME'] == selected_village].iloc[0]

# Extra fallback safety checks per village row
temp_val = village_data['CURRENT_TEMP_C'] if 'CURRENT_TEMP_C' in village_data.index else 11.5
if pd.isna(temp_val) or (isinstance(temp_val, float) and np.isnan(temp_val)):
    temp_val = 11.5

rain_val = village_data['CURRENT_RAIN_MM'] if 'CURRENT_RAIN_MM' in village_data.index else 0.0
if pd.isna(rain_val) or (isinstance(rain_val, float) and np.isnan(rain_val)):
    rain_val = 0.0

condition_val = village_data['CURRENT_CONDITION'] if 'CURRENT_CONDITION' in village_data.index and pd.notna(village_data['CURRENT_CONDITION']) else "Clear Sky"
advisory_val = village_data['AGRICULTURAL_ADVISORY'] if 'AGRICULTURAL_ADVISORY' in village_data.index and pd.notna(village_data['AGRICULTURAL_ADVISORY']) else "STABLE/DRY CONDITIONS: Winter framework active."
...
with col1:
    ...
    m1.metric("Altitude", f"{int(village_data['RASTERVALU'])} m" if 'RASTERVALU' in village_data.index else "1654 m")
    m2.metric("Current Temp", f"{temp_val} °C")
    m3.metric("Live Rainfall", f"{rain_val} mm")
    
    # Contextual geographic categorizations
    st.write(f"**District:** {village_data['DISTRICT'] if 'DISTRICT' in village_data.index else 'Butha-Buthe'}")
    st.write(f"**Terrain Zone:** {village_data['CLIMATE_ZONE'] if 'CLIMATE_ZONE' in village_data.index else 'Valley'}")
    st.write(f"**Baseline Frost Risk:** {village_data['FROST_RISK'] if 'FROST_RISK' in village_data.index else 'Low'}")
