...
village_data = df[df['NAME'] == selected_village].iloc[0] if len(df[df['NAME'] == selected_village]) > 0 else df[df['NAME'] == selected_village].iloc

# --- MILESTONE 1: TERRAIN RAINFALL POTENTIAL MODEL ---
# Using physical geography principles to calculate orographic rainfall index based on DEM values
...
def fetch_live_weather(lat, lon):
    ...
    try:
        ...
        pass
    return {"temp": 5.8, "rain": 0.0, "condition": "Winter Air Normal"}
...
with col1:
    ...
    st.markdown("---")
    # NEW MILESTONE 1 DISPLAY LAYOUT INTERFACE
